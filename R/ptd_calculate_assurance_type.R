...
ptd_calculate_assurance_type <- function(.data) {
  d <- .data %>%
    group_by(.data$f) %>%
    #slice_tail(n = 1)
    # Alter to perform same function pre dplyr 1.1.0
    do(tail(., 1))
...
ptd_calculate_assurance_type_2 <- function(.data, improvement_direction) {
  d <- .data %>%
    group_by(.data$f) %>%
    #slice_tail(n = 1)
    # Alter to perform same function pre dplyr 1.1.0
  ...
  if (is.null(unique(.data$target)) || improvement_direction == "neutral") {
    return(summarise(d, assurance_type = as.character(NA), .groups = "drop"))
  }

  # linting reports this is assigned by not used, so excluding line from linting as it is used
  is_increasing <- improvement_direction == "increase" # Exclude Linting
