...
validateParameters <- function(df, valueField, dateField, facetField, options){
  ...
  if(!is.null(options$rebase)){
    if(!is.character(options$rebase)) stop("spc: options$rebase argument must be of type 'character'.")
    if(length(options$rebase) > 1) stop("spc: options$rebase argument must be a vector of length 1.")
  ...
  if(!is.null(options$outputChart)){
    if(!is.logical(options$outputChart)) stop("spc: options$outputChart argument must be TRUE or FALSE.")
  }

  #if provided, options$pointSize must be a number between 0 and 10
  if(!is.null(options$pointSize)){
    if(!is.numeric(options$pointSize)) stop("spc: options$pointSize argument must be a number.")
    if(options$pointSize <= 0 || options$pointSize > 10) stop("spc: options$pointSize argument must greater than 0 and less than or equal to 10.")
  }
  ...
  if(!is.null(options$percentageYAxis)){
    if(
      !is.logical(options$percentageYAxis) &
  ...
  if(!is.null(options$target)){
    if(!is.character(options$target)) stop("spc: options$target argument must be of type 'character'.")
    if(length(options$target) > 1) stop("spc: options$target argument must be a vector of length 1.")
    if(!(options$target %in% colnames(df))) stop("spc: options$target argument must be a column name from the data frame.")
  }
  ...
  if(!is.null(options$trajectory)){
    if(!is.character(options$trajectory)) stop("spc: options$trajectory argument must be of type 'character'.")
    if(length(options$trajectory) > 1) stop("spc: options$trajectory argument must be a vector of length 1.")
    if(!(options$trajectory %in% colnames(df))) stop("spc: options$trajectory argument must be a column name from the data frame.")
  }
  ...
  if(!is.null(options$fixedXAxisMultiple)){
    if(!is.logical(options$fixedXAxisMultiple)) stop("spc: options$fixedXAxisMultiple argument must be TRUE or FALSE.")
  }

  #if provided, options$fixedYAxisMultiple must be TRUE or FALSE
  if(!is.null(options$fixedYAxisMultiple)){
    if(!is.logical(options$fixedYAxisMultiple)) stop("spc: options$fixedYAxisMultiple argument must be TRUE or FALSE.")
  }

  #if provided, options$xAxisDateFormat must be of character type
  if(!is.null(options$xAxisDateFormat)){
    if(!is.character(options$xAxisDateFormat)) stop("spc: options$xAxisDateFormat argument must be of type 'character'. Try ?strptime for formatting hints.")
  }

  #if provided, options$xAxisBreaks must be of character type, and contain one of "day", "week", "month", "quarter" or "year"
  if(!is.null(options$xAxisBreaks)){
    if(
      !is.character(options$xAxisBreaks) ||
      (
      !grepl("day", options$xAxisBreaks, fixed=TRUE) &
      !grepl("week", options$xAxisBreaks, fixed=TRUE) &
      !grepl("month", options$xAxisBreaks, fixed=TRUE) &
      !grepl("quarter", options$xAxisBreaks, fixed=TRUE) &
      !grepl("year", options$xAxisBreaks, fixed=TRUE)
      )
    ) stop("spc: options$xAxisBreaks argument must be of type 'character', and be a valid string for seq.Date 'by'.  See seq.Date for more information.")
  }
