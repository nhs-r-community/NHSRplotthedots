...
test_that("it returns a data frame", {
  ...
  r <- ptd_spcStandard(data, list(valueField = "y", dateField = "x"))
...
test_that("it sets the trajectory field", {
  ...
  r1 <- ptd_spcStandard(data, list(valueField = "y", dateField = "x", trajectory = "t"))
  expect_equal(r1$trajectory, 1:20)

  # when options$trajectory is not set
  r2 <- ptd_spcStandard(data, list(valueField = "y", dateField = "x"))
...
test_that("it sets the target field", {
  ...
  r1 <- ptd_spcStandard(data, list(valueField = "y", dateField = "x", target = "t"))
...
test_that("it sets the rebaseGroup field", {
  ...
  # when options$target is set
  r1 <- ptd_spcStandard(data, list(valueField = "y", dateField = "x", rebase = "t"))
...
test_that("setting fixAfterNPoints changes the calculations", {
  ...
  s0 <- ptd_spcStandard(data, ptd_spcOptions("x", "y"))
  s1 <- ptd_spcStandard(data, ptd_spcOptions("x", "y", fixAfterNPoints = 12))
