...
test_that("dateField must be either a Date or POSIXt vector", {
  o <- spcOptions("b", "a")
  validateSpcOptions(o, data.frame(a = Sys.Date(), b = 1))
  validateSpcOptions(o, data.frame(a = Sys.time(), b = 1))

  expect_error(validateSpcOptions(o, data.frame(a = 1, b = 1)),
               "dateField must be a Date or POSIXt vector ('a' is a 'numeric').",
               fixed = TRUE)
  expect_error(validateSpcOptions(o, data.frame(a = "a", b = 1)),
